...
협력 객체는 객체 지향 프로그래밍에서 특정 객체가 자신의 역할을 수행하기 위해 상호작용하는 다른 객체를 의미합니다. 객체는 모든 작업을 직접 처리하지 않고 협력 객체에 위임하여 책임을 분산시킵니다.

### 주요 특징

**책임 분리**

* 각 객체는 자신의 고유한 책임만을 가지며, 필요한 작업은 협력 객체에 위임
* 단일 책임 원칙(SRP)을 준수하여 유지보수성 향상

**캡슐화 유지**

* 객체 내부 구현을 숨기고 필요한 인터페이스만 노출
* "Tell, Don't Ask" 원칙을 통해 객체 간 결합도 감소
